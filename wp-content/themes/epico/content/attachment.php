<article <?php hybrid_attr( 'post' ); ?>>
	...
	<?php else : // Se NAO estiver vendo um anexo unico. ?>
		...
		<?php if ( has_post_thumbnail() ) : ?>
			...
			<a href="<?php the_permalink() ?>" title="<?php the_title() ?>">1

				<?php the_post_thumbnail() ?>2
		...
		<header class="entry-header">
			...
			<?php the_title( '<h2 ' . hybrid_get_attr( 'entry-title' ) . '><a href="' . get_permalink() . '" rel="bookmark" itemprop="url">', '</a></h2>' ); ?>3

		</header><!-- .entry-header -->

		<div <?php hybrid_attr( 'entry-summary' ); ?>>4
