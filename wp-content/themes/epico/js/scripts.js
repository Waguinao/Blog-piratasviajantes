...
var FlightTable = new(function(){
    var that = this;

    that.onClick = function(url){
        window.open(url,'_blank');
    }
});

var FilterFlight = function(){
	var that = this;
	...
	that.bindEvents = function(){
		$('#filter-ori').on('change', that.onFilterOriChange);
		$('#filter-dest').on('change', that.onFilterDestChange);
	}

	that.onFilterOriChange = function(){
		var valOri = $('#filter-ori').val();
		var valDest = $('#filter-dest').val();

		if(valOri == ""){

			if(valDest == ""){
				$('#filter-table tbody tr').removeClass('hide');
			}else{
				$('#filter-table tbody tr[dest="'+ valDest +'"').removeClass('hide');
			}

		}else{

			if(valDest != ""){
				$('#filter-table tbody tr').addClass('hide');
				$('#filter-table tbody tr[ori="'+ valOri +'"][dest="'+ valDest +'"]').removeClass('hide');
			}else{
				$('#filter-table tbody tr').addClass('hide');
				$('#filter-table tbody tr[ori="'+ valOri +'"]').removeClass('hide');
			}		

		}
	}

	that.onFilterDestChange = function(){
		var valOri = $('#filter-ori').val();
		var valDest = $('#filter-dest').val();
...
(function(factory) {  // Try to register as an anonymous AMD module
	...
	var services = {
		...
		twitter: {
			counterUrl: 'http://opensharecount.com/count.json?url={url}&callback=?',
			convertNumber: function(data) {
				return data.count;
			},
