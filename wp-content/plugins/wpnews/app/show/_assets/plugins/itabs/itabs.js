...
/**
 * é executado caso a função getElementsByClassName não exista (IE)
 *
 */
(function() {
...
(function(){
	...
	var Functions = {
		...
		openTab:function(dataFor,mode,idTabNow){
			dataForOriginal = dataFor; 

			 var operatorIgual = dataFor.indexOf('=');                   
                if(operatorIgual != -1){                    	
                	dataFor = dataForOriginal.substring(0,operatorIgual);                    			
                	dataForValue = dataForOriginal.substring(operatorIgual+1);                    			                	
                }	
			...
			if(dataHash == "true" || dataHash == true){
                 ...
                 if(mode != 'init')
					 window.location = "#"+dataForOriginal;
			}
