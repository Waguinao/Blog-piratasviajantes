...
(function(){
	...
	function MeTabs(){
		...
		this.next = function(obj){
			...
			var total = bot.parents('div').find('[data-rel="tabContainer"] > a').length-1;
			var index = bot.index();
		...
		this.checkShow = function(thiss,obj,idTabContent){
			...
			var total = obj.parents('div').find('div[data-rel="metabs_content"]').length;
			...
			for(var a = 0;a<= rules.animateStop.length-1;a++){	
				...
				obj.parents('div').find('div[data-rel="metabs_content"]').stop();	
		...
		this.changeContent = function(obj,idTabContent){
			...
			for(var i = 0;i<= rules.animateEnd.length-1;i++){		
				...
				obj.parents('div').find('div[data-rel="metabs_content"]').animate(rules.animateEnd[i],rules.animateEnd[i].duration,function(){				
					...
					if(index == total){
						...
						parent.checkShow(this,obj,idTabContent);
					}			
					index++;
				});				
			}
