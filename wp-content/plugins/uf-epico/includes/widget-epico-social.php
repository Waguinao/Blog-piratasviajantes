...
class Widget_social extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			array(
				'description'	=>	__( 'Easily add social buttons to your theme.', 'uf-epico' )
			)
		,array( 'width' => 495)
	...
	public function register_admin_styles_scripts() {
		...
		//$configfiles = glob( self::get_path( dirname( __FILE__ ) ) .'configs/social-*.php' );
		if(file_exists(self::get_path( dirname( __FILE__ ) ) .'configs/fieldgroups-social.php')){
			include self::get_path( dirname( __FILE__ ) ) .'configs/fieldgroups-social.php';
		}else{
			return;
...
add_action( 'widgets_init', create_function( '', 'register_widget("Widget_social");' ) );
