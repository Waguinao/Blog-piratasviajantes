...
class Widget_epico_notice extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			'epico_notice-id',
			__( 'Epico Aviso', 'uf-epico' ),
			array(
				'description'	=>	__( 'Create a cool notice at the top of your website.', 'uf-epico' )
			)
		,array( 'width' => 495)
				...
				if(is_active_sidebar($sidebarid) && $sidebarid != 'wp_inactive_widgets' && false === strpos($sidebarid, 'orphaned_widgets')){
					foreach($widget_settings as $key=>$settings){
						if(in_array($this->id_base.'-'.$key,$sidebar)){
							...
							wp_enqueue_script( 'epico_notice-jquerycookieminjs', '//cdn.jsdelivr.net/jquery.cookie/1.4.0/jquery.cookie.min.js', false , false, true );
