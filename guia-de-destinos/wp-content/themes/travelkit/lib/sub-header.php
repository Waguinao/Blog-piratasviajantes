<?php 
    ...
    $sub_img = array();
    global $post;
    if(!function_exists('travelkit_call_sub_header')){
        function travelkit_call_sub_header(){
                ...
                if(!empty($image_attached)){
                    ...
                    if(empty($sub_img[0])){
                        ...
                        if( get_post_meta(get_the_ID(),"themeum_subtitle_color",true ) == ''){
                            $output = travelkit_call_sub_header();
                        }
                    }
                }else{
                    if( get_post_meta(get_the_ID(),"themeum_subtitle_color",true) != "" ){
                    ...
                    <?php
                        ...
                        if (get_theme_mod( 'sub_header_title_enable', true )) {
                            ...
                            if(isset($wp_query->queried_object->name)){
                                ...
                                if( is_search() ){
                                    $first_char = esc_html__('Search','travelkit');
                                    echo '<h2 class="page-leading">'.$first_char.'</h2>';
                                }

                                else if( is_home() ){
                                        echo '<h2>'.esc_html__('Blog','travelkit').'</h2>';
                                    }
                                else if( is_single()){} 
